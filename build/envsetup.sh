...
cat <<EOF
Additional Cardinal-AOSP functions:
...
- crdmka:          Cleans and builds using mka.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the Cardinal-AOSP model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
function makerecipe() {
    ...
    then
        pwd
        crdremote
        git push crd HEAD:refs/heads/'$1'
    fi
    '
}
...
alias crdkap='dopush crdmka'

function repopick() {
    T=$(gettop)
    $T/vendor/cardinal/build/tools/repopick.py $@
}
